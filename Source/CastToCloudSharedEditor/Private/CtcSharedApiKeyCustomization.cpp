...
FCtcSharedApiKeyStatus::FCtcSharedApiKeyStatus() : Result(MakeValue(TEXT("")))
{
}

void FCtcSharedApiKeyStatus::Reset()
{
	Result = MakeValue(TEXT(""));
}

bool FCtcSharedApiKeyStatus::IsEmpty() const
{
	return Result.HasValue() && Result.GetValue() == TEXT("");
}

FText FCtcSharedApiKeyStatus::GetDisplayText() const
{
	const FString ResultString = Result.HasValue() ? Result.GetValue() : Result.GetError();
	return FText::FromString(ResultString);
}

FLinearColor FCtcSharedApiKeyStatus::GetDisplayColor() const
{
	return Result.HasValue() ? FLinearColor::Green : FLinearColor::Red;
}

TSharedRef<IPropertyTypeCustomization> FCtcSharedApiKeyCustomization::MakeInstance()
{
	return MakeShared<FCtcSharedApiKeyCustomization>();
}

void FCtcSharedApiKeyCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> StructProperty, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
}

void FCtcSharedApiKeyCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> StructProperty, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	TSharedPtr<IPropertyHandle> ApiKeyProperty = StructProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FCtcSharedApiKey, ApiKey));
			...
			[
				...
				[
					...
					.IsPassword_Lambda([this, ApiKeyProperty]()
					{
						...
						return !Value.IsEmpty() && Value != FCtcSharedApiKey::PlaceholderKey;
					})
...
void FCtcSharedApiKeyCustomization::Tick(float DeltaTime)
{
...
void FCtcSharedApiKeyCustomization::SendApiKeyPermissionsRequest(const FString& ApiKey)
{
	if (ApiKey == FCtcSharedApiKey::PlaceholderKey || ApiKey.IsEmpty())
	{
	...
	Request->OnProcessRequestComplete().BindRaw(this, &FCtcSharedApiKeyCustomization::OnApiKeyPermissionsRetrieved, ApiKey);
	Request->ProcessRequest();
}

void FCtcSharedApiKeyCustomization::OnApiKeyPermissionsRetrieved(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful, FString ApiKeyUsed)
{
	...
	FCtcSharedApiKeyPermissionResponse Result;
